...
import { IPortfolio, portfolioData } from "@/data/portfolio-data";
...
export default function PortfolioDetailsArea({ portfolio }: Props) {
  const currentIndex = portfolioData.findIndex((p) => p.id === portfolio.id);
  const prevPortfolio =
  ...
  const mainImage = galleryImages[0] || portfolio.image;
  const secondImage = galleryImages[1] || galleryImages[0] || portfolio.image;
  return (
    <section className="work-details-area">
          ...
          <div className="meta-wrapper fade-anim">
            {portfolio.service && (
              <div className="meta-item">
                ...
                <p className="text">{portfolio.service}</p>
              </div>
            )}
            {portfolio.client && (
              <div className="meta-item">
                <p className="title">Client</p>
                <p className="text">{portfolio.client}</p>
              </div>
            )}
            <div className="meta-item">
              <p className="title">Date</p>
              <p className="text">{portfolio.date}</p>
            </div>
            {portfolio.technology && (
              <div className="meta-item">
                <p className="title">Technology</p>
                <p className="text">{portfolio.technology}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {galleryImages.length > 2 && (
          <div className="gallery-wrapper fade-anim">
            {galleryImages.slice(2).map((image, index) => (
              <div key={index} className="image parallax-view">
                <Image
                  src={image}
                  alt={`${portfolio.title} - Gallery ${index + 3}`}
                  width={840}
                  height={580}
                  data-speed="0.8"
                  style={imgStyle}
      ...
      <div className="container large">
        <div className="pagination fade-anim">
          {prevPortfolio && (
            <Link href={`/portfolio-details/${prevPortfolio.slug}`}>
              <svg
                width="13"
                height="18"
                viewBox="0 0 13 18"
          ...
          {nextPortfolio && (
            <Link href={`/portfolio-details/${nextPortfolio.slug}`}>
              Next
              <svg
                width="13"
                height="18"
